...
def main():
    ...
    # Position of instance in dataset.
    c = 1
    ...
    for user in user_codes:
        ...
        if labels.shape[1] == 2:
            # Change sample size to create different time differences.
            labels = labels.set_index('resp_time').resample('3h').max()
            labels = labels[labels.STRESSED.notnull()]
        else:
            continue
        ...
        print('Empty data filled.')
        
        df.set_index('timestamp')
        df.index = pd.to_datetime(df.index, unit='s')

        # Resample data (Change resample value to create new different time ranges.)
        df = df.resample('15min').max()
        df = df.fillna(0)
        df['activity_inference'] = df.activity_inference.astype('category')
        df['audio_inference'] = df.activity_inference.astype('category')

        df = pd.get_dummies(df)

        instance, c = create_instances(df, labels, c)

        dataset.update(instance)
        
        print("User:", user, "is completed.")

    np.save('student_life_only_activity_dataset.npy', dataset)
